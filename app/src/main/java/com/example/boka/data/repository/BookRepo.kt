...
class BookRepo(private val bookService: BookService) : BaseRepo() {
    ...
    suspend fun getItemBasedBook(bookId: Int): NetworkResult<List<Book>> {
        val res: NetworkResult<BodyResult<List<BookJson>>> = bookService.getItemBasedBook(bookId)
        return handleNetworkResult(res) { it.data.map { bookJson -> bookJson.toBook() } }
    }
    ...
    suspend fun getNewUserBooks(): NetworkResult<List<Book>> {
        val res: NetworkResult<BodyResult<List<BookJson>>> = bookService.getNewUserBooks()
        return handleNetworkResult(res) { it.data.map { bookJson -> bookJson.toBook() } }
    }
